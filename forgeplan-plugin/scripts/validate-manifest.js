...
/**
 * Minimal YAML parser sufficient for ForgePlan manifests.
 * Handles: scalars, sequences (- items), nested maps, quoted strings, flow sequences [a, b].
 * Does NOT handle: anchors, tags, multi-doc, complex keys.
 */
...
function parseBlock(lines, startIdx, minIndent) {
  ...
  while (i < lines.length) {
    ...
    // Sequence item at current level
    if (stripped.trim().startsWith("- ")) {
      // This is a sequence — back up and parse as sequence
      const seq = parseSequence(lines, i, indent);
      // If we're inside a map, this shouldn't happen at top level
      // Return what we have — the caller handles sequences
      return { value: result, nextIdx: i };
    }

    // Key: value pair
    const colonMatch = stripped.match(/^(\s*)([\w$.\-]+)\s*:\s*(.*)/);
    if (colonMatch) {
      const key = colonMatch[2];
      let val = colonMatch[3].trim();
        ...
        if (nextNonEmpty < lines.length) {
          ...
          if (nextIndent > indent) {
            ...
            if (lines[nextNonEmpty].trim().startsWith("- ")) {
              ...
              if (val === ">") {
                // Folded scalar — collect until dedent
                let scalar = "";
                let j = i + 1;
                while (j < lines.length) {
                  const sLine = lines[j].replace(/\r$/, "");
                  if (sLine.trim() === "") {
                    scalar += "\n";
                    j++;
                    continue;
...
function parseSequence(lines, startIdx, seqIndent) {
  const items = [];
  let i = startIdx;
  ...
  while (i < lines.length) {
    ...
    if (indent > seqIndent && !line.trim().startsWith("- ")) {
      // Continuation of a nested structure under last sequence item
      i++;
      continue;
    }
    ...
    // Sequence item is a map (- key: value)
    if (after.match(/^[\w$.\-]+\s*:/)) {
      // Multi-line map item
      const mapItem = {};
      // Parse first key
      const firstMatch = after.match(/^([\w$.\-]+)\s*:\s*(.*)/);
      if (firstMatch) {
        mapItem[firstMatch[1]] = parseScalar(firstMatch[2].trim());
      }
      ...
      while (i < lines.length) {
        ...
        if (contIndent <= seqIndent) break;
        if (contLine.trim().startsWith("- ") && contIndent === seqIndent)
          break;

        const contMatch = contLine
          .trim()
          .match(/^([\w$.\-]+)\s*:\s*(.*)/);
        if (contMatch) {
          mapItem[contMatch[1]] = parseScalar(contMatch[2].trim());
        }
        i++;
      }
      items.push(mapItem);
...
if (require.main === module) {
  ...
  module.exports = { validateManifest, parseYaml, detectCycles, scopesOverlap };
}
