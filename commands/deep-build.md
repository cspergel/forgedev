...
7. On success, delete `.forgeplan/phase-advance-backup/`. Do NOT clear `sweep_state` or `sweep_state.phase_advancement` yet.
8. Start new build cycle for promoted nodes (loop back to Phase 2). Phase 2's initialization overwrites `sweep_state` with fresh `operation: "deep-building"`, `current_phase: "build-all"` — this is when the old phase_advancement data gets replaced. A crash between step 7 and step 8 is safe because sweep_state still contains the phase_advancement breadcrumb with `checkpoint: "promoting_specs"` and `promoted_nodes`, allowing recovery to detect that promotion succeeded and the next step is the build loop.
