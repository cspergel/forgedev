...
7. Do NOT delete backups or clear sweep_state yet. Proceed directly to step 8.
8. Start new build cycle for promoted nodes (loop back to Phase 2). Phase 2's initialization overwrites `sweep_state` with fresh `operation: "deep-building"`, `current_phase: "build-all"`. **After Phase 2 successfully initializes sweep_state**, delete `.forgeplan/phase-advance-backup/` (safe now — recovery no longer needs it because sweep_state has been reset to the build loop). A crash before Phase 2 init is safe: sweep_state still has the phase_advancement breadcrumb with `checkpoint: "promoting_specs"`, `promoted_nodes`, and the backup dir — recovery can detect promotion succeeded and restart the build loop.
