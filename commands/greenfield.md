...
1. If `.forgeplan/deep-build-report.md` exists AND `sweep_state` is null in state.json AND all nodes have status `"built"` or `"reviewed"` AND no node has `bounce_exhausted: true` → **project is already complete**. Present the final output and exit. If any node has `bounce_exhausted: true` with `unverified_acs`, log: "Project completed with unverified acceptance criteria on [node]. Run `/forgeplan:review [node]` to verify." Still present final output but include the warning.
