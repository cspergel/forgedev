...
function evaluateBash(toolInput, cwd) {
  ...
  const safePatterns = [
    ...
    nodeScriptPattern("iso-now.js"),               // deterministic timestamp helper for sweep/deep-build artifacts
    nodeScriptPattern("load-sweep-findings.js", String.raw`(?:\s+(?:--stdin|".*?"|'.*?'|\S+))?(?:\s|$)`), // deterministic sweep findings ingestion
